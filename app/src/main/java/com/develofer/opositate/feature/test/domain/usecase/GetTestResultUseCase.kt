...
class GetTestResultUseCase @Inject constructor(
    ...
    suspend operator fun invoke(solvedTestId: String): Result<SolvedTest> {
        ...
        val testAbilityId = (testSolvedResult).data?.abilityId ?: 0
        val testTaskId = (testSolvedResult).data?.taskId ?: 0
        val testResult = getTestUseCase(testResultId, testAbilityId, testTaskId)
        ...
        return if (testSolved == null || test == null) Result.Error(Exception(resourceProvider.getString(R.string.error_message__test_data_missing)))
                ...
                Result.Success(
                    SolvedTest(
                        ...
                        name = "${test.id + 1}. ${resourceProvider.getString(getTaskStringResIdUseCase(testAbilityId, testTaskId))}",
